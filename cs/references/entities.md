...
- **启动和澄清**：先读事实，再把没想清楚的想法整理到 talks，或把上游规格澄清到 specs。
- **事项**：记录这次要做什么，做完就关闭。
...
读取 facts 遵循上下文复用。当前对话或执行上下文里没读过就读；已经读过且没有修改迹象，就复用已知事实，不要求每个技能机械重读。
...
上游规格澄清工作区。它承接 PM/SE/PRD/会议材料这类目标大体稳定、但规格传递不完整的输入，把未知系统、术语、流程、阻碍和变更记录下来。它不要求一次澄清到全局完美；某一批范围足够明确时，就可以先交给 `cs-plan`。

specs 不是 requirements。它记录一次规格接入和澄清过程；正式 requirements 只接收关闭事项后沉淀的当前真相。specs 也不是 wiki，wiki 是长期系统理解，specs 是围绕某个上游任务目标的工作区。

spec 按日期分片并以目录存放：`.cs/specs/YYYY/MM/DD/{slug}/`。主文档是 `index.md`；调查资料放 `findings/`；交给 `cs-plan` 的入口是 `plan-brief.md`。规格补充、修改或推翻已实现内容时，回到同一个 spec 工作区继续更新，不另起一套事实。

默认结构：

- `index.md`：规格目标、用户需求复述、整体流程、关键阻碍、本轮可计划判断。
- `source.md`：上游规格材料来源、版本、会议材料和摘要。
- `questions.md`：待确认问题、已确认答案、阻塞级别。
- `glossary.md`：规格里的系统名、业务词、角色、状态和确认程度。
- `flow.md`：端到端流程、状态变化、异常路径和破坏性变更。
- `findings/{topic}.md`：未知系统/概念/流程/依赖的一事一篇调查资料。
- `changes.md`：规格增删改查、会议结论、推翻点、影响范围和受影响事项。
- `plan-brief.md`：给 `cs-plan` 的入口材料，标明本轮可计划范围和暂不计划范围。

模板：`templates/entities/spec-index.md`、`spec-source.md`、`spec-questions.md`、`spec-glossary.md`、`spec-flow.md`、`spec-finding.md`、`spec-changes.md`、`spec-plan-brief.md`
