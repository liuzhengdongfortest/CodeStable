...
- 风险 6：根目录遗留独立 skill 会让 `skills CLI --list` 暴露非 CodeStable 资产。缓解：本 PR 直接删除根目录 `browser-bridge/`，并让 `check-plugin-package` 对根目录任意 `SKILL.md` skill 返回失败。
...
- `browser-bridge/` 曾是仓库根的非 `cs*` skill；本 PR 直接删除，不迁入 CodeStable 插件实体。
...
- 边界：非 `cs*` skill 不进入插件实体；根目录也不再保留独立 skill，若根目录出现任意带 `SKILL.md` 的 skill 目录，校验失败。
...
- 生成资产不应包含非 `cs*` skill、`.pytest_cache/`、`__pycache__/`、`*.pyc`、`.DS_Store`。
...
- 根目录独立 skill 已从本 PR 移除；后续若需要恢复类似能力，应作为独立 plugin / skill repo 重新设计，不放回 CodeStable 分发根目录。
