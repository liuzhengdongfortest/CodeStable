...
- **review transport**：同 workspace reviewer 直接接收 canonical locator 并回源，不构建重复全文 packet；只有 remote / file handoff 才用 `build-review-packet.py --transport portable`，且必须传显式 `--include-path` allowlist。大 diff / 报告经文件路径传递，不粘进派发 prompt 或回传正文；不含 `.env`、token、secret。
