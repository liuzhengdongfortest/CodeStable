...
大需求放在 `.cs/epics/YYYY/MM/DD/{短语}/`。一个 epic 目录包含 `index.md`、`spec.md`、`plan.md`：导读、当前规格和架构考量、本轮推进视图。实现中规格补充、修改或推翻，都回到同一个 epic spec，而不是新建流水 `changes.md`。
...
<tr><td><b>讨论/规划入口</b></td><td><code>cs-talk</code></td><td>产品/PL 入口：讨论清楚真实问题、约束和边界，做初步规划；用户确认后写入 <code>talks/</code>，或创建独立 issue / 新 epic 骨架</td></tr>
<tr><td><b>规格入口</b></td><td><code>cs-spec</code></td><td>维护 project spec 或 epic 活规格：需求、架构考量、统一语言、本轮推进视图和未确认问题</td></tr>
...
═══════════════════════════════════════════════════════════════
                          │
        ┌─────────────────┼─────────────────┐
   （未接入）          （想法/规划）        （spec 需要更新）       （明确 issue）
   cs-onboard         cs-talk ─────┐      cs-spec ─────┐      cs-design ─▶ cs-do ─▶ cs-close
   搭骨架              查上下文 + talks│      project/epic│      设计 issue → 执行验证 → 按归属沉淀
                       初步规划并确认后 └───────────────┴────▶ 独立 issue / epic / epic issue
                       cs-complain ─▶ 行为跑偏时完成 bug 诊断修复闭环
═══════════════════════════════════════════════════════════════
 spec · 当前真相和活规格边界           （.cs/spec/ 和 .cs/epics/）
───────────────────────────────────────────────────────────────
   project spec ─▶ 项目是什么 / 往哪走 / 能力地图 / 架构地图 / 统一语言 / 阅读路径
   epic spec    ─▶ 大需求改变什么 / 为什么这样设计 / 本轮推进视图 / 未确认问题
