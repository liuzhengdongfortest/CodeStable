...
### requirements —— 需求、约束、取舍

这是 CodeStable 的北极星：**读它即知代码背后的需求、约束与取舍**。它只记代码本身展示不出来的东西——为什么这样、为什么不那样、被否决的方案、哪些用户故事必须守住、哪些边界会变化。代码讲"做了什么"，requirements 补"为什么必须这样"，合起来才是完整图景，谁都不重复谁。

- **`cs-requirements`** —— 当前需求、约束、领域词汇和取舍说明（happy path / 边界条件 / 为什么需要灵活性）。不引用代码位置、不记历史叙事

### 二者怎么咬合

事项是增量，requirements 是这些增量沉淀出的当前需求真相。**一个 issue 或 epic 关闭时，把毕业的约束、事实和取舍回写 requirements。** requirements 不记历史（那在关闭的 issue 里），issue 不长期描述当前需求。写代码的纪律（`cs-code`）和知识笔记（`cs-keep`）服务两者。
...
<tr><td><b>接入</b></td><td><code>cs-onboard</code></td><td>把 CodeStable 接入仓库：搭骨架 + 分发共享资产 + 归旧档</td></tr>
...
<tr><td rowspan="4"><b>事项</b></td><td><code>cs-issue</code></td><td>一件可关闭的变更：bug / 重构 / 小功能 / 杂务，tag 分类型</td></tr>
...
<tr><td><code>cs-code</code></td><td>写代码的纪律：只写当前明确要的、漂移那刻停（任何动手都用）</td></tr>
<tr><td><b>需求</b></td><td><code>cs-requirements</code></td><td>当前需求、约束、领域词汇和取舍说明（happy path / 边界 / 为什么需要灵活性）</td></tr>
<tr><td rowspan="2"><b>辅助资料</b></td><td><code>cs-keep</code></td><td>坑点 / 技巧 / 决策 / 调研沉淀到 <code>notes/</code>，纯 markdown，全文检索</td></tr>
...
═══════════════════════════════════════════════════════════════
        ...
        ┌─────────────────┼─────────────────┐
   （未接入）          （想法模糊）        （已接入）
   cs-onboard         cs-talk ─▶ cs-plan   直达事项 / requirements
   搭骨架              整理 talks → 生成 epics/issues
═══════════════════════════════════════════════════════════════
...
───────────────────────────────────────────────────────────────
   cs-requirements ──▶ 当前需求 + 领域词汇 + 取舍说明
...
- `requirements/` 承载当前需求、约束、领域词汇和取舍，只描述当前真相、不记历史叙事；历史在关闭的 issue 里
- 事项默认写入本地 `issues/ epics/`，先保持一条清晰主线
- `notes/` 是知识笔记，纯 markdown 无 frontmatter，靠全文检索——好写好搜
- `reference/` 由 `cs-onboard` 从技能包分发；体系口径由 `cs` 技能及其 references/templates 承载

### 硬约束

> Skill 是独立安装单元，运行时**每个 skill 只能看到自己包内的文件**。A 技能的 SKILL.md 里写 `B-skill/reference/xxx.md` 这种引用在运行时**根本读不到**。
>
> 跨 skill 共享的参考文档必须走"工作项目"这一层：由 `cs-onboard` 从技能包分发到 `.cs/reference/`，其他 skill 用项目相对路径读取。

要改体系口径，更新 `cs` 技能及其 references/templates；新项目 onboard 时带上新版本，已有项目重跑 `cs-onboard` 同步共享参考。
