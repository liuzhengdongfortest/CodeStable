...
<p>
  ...
  <img src="https://img.shields.io/badge/skills-18-6366F1?style=flat-square" alt="Skills"/>  <img src="https://img.shields.io/badge/license-MIT-10B981?style=flat-square" alt="License"/>
</p>
...
- **`cs-spec`** —— 接住上游规格/PRD/SE 文档：目标稳定但信息不完整时，调查未知系统、术语和实现阻碍，支持多轮澄清和分批 `plan-brief`
- **`cs-plan`** —— 接住 `talks/` 或 spec `plan-brief`：小需求直接落成 issue，大需求先进 epic 规划并拆 issues
...
<tr><td><b>规格入口</b></td><td><code>cs-spec</code></td><td>上游规格/PRD/SE 文档目标稳定但信息不完整时，澄清规格、调查未知系统和阻碍，产出 <code>.cs/specs/</code> 并标出本轮可计划范围</td></tr>
<tr><td><b>投诉入口</b></td><td><code>cs-complain</code></td><td>行为不符合预期时，建 bug issue，建立反馈回路，诊断根因，修复验证并回写</td></tr>
<tr><td><b>计划入口</b></td><td><code>cs-plan</code></td><td>读取 <code>talks/</code> 或 spec plan-brief，判断直接进入 issue 还是先进入 epic，并生成对应草案</td></tr>
...
<tr><td><code>cs-how-docs</code></td><td>把 wiki、requirements、notes、README 等组织成可阅读的知识空间，而不是平铺内容</td></tr>
...
═══════════════════════════════════════════════════════════════
   ...
   搭骨架              查上下文 + talks│      specs + brief│      生成事项 → 设计 issue → 可选测试 → 执行验证 → 关闭沉淀
                                  └───────────────┴────▶ 直达事项 / requirements
                       cs-complain ─▶ 行为跑偏时完成 bug 诊断修复闭环
═══════════════════════════════════════════════════════════════
 事项 · 要做、做完会关闭的事           （.cs/issues/ 或 .cs/epics/）
───────────────────────────────────────────────────────────────
   cs-spec   ──▶ 上游规格渐进澄清：未知系统/术语/流程调查 → 本轮 plan-brief
   cs-plan   ──▶ 从 talks 或 spec 本轮可计划范围判断：直接 issue，或先进 epic 再拆 issues
...
───────────────────────────────────────────────────────────────
   requirements ──▶ index 主文档 + 子文档说明 + 领域词汇 + 取舍
                    （背景 / 功能 / 关键规则 / 边界）
                    北极星：读它即知代码背后的需求与取舍
...
- `specs/` 承接目标稳定但信息不完整的上游规格，先调查未知系统、术语和流程；某一批范围足够清楚就写进 `plan-brief.md` 交给 `cs-plan`，剩下的继续澄清
...
- talks / epics / notes 默认写入 `YYYY/MM/DD/{slug}.md` 日期分片，specs 写入 `YYYY/MM/DD/{slug}/` 工作区，issues 写入 `YYYY/MM/DD/{status}-{slug}.md`；查找时递归搜索对应目录
