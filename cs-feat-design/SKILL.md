...
外加一个**固定结构健康度环节**（第 2.5 节）：评估即将被改动的文件是否偏胖 / 职责混杂，决定是否在实现前先做"只搬不改行为"的微重构。即使结论是"不做"也要在 design 里显式写出来，**用户必须在整体 review 前对这一节单独表态**——否则 AI 默认会持续往胖文件里塞代码。
...
- **写完第 2.5 节就单独和用户对齐一次**："文件 X 我判断属于 {结论}，{理由}，{方案}，是否同意？"——用户确认后再补齐其余节进整体 review。不做这一步，第 5 步整体 review 时用户容易把微重构方案和功能方案混在一起看，反而判断不清
...
- [ ] 第 2.5 结构健康度结论显式写出（不做 / 微重构方案），且已单独获得用户确认；选"微重构"时 checklist 第 1 步是它且有独立退出信号；超出"只搬不改行为"的结构性问题列在"超出范围的观察"，仅提示不阻塞
