...
本技能在正文里用自然语言术语引用路径（**技巧库目录**），具体目录位置的**权威定义**见主技能 `easysdd` 第二节"目录安排"。本节只说文档命名规则。

**文件命名**：`YYYY-MM-DD-{slug}.md`（日期取**归档当天**，slug 为英文小写 + 连字符）。
...
按以下顺序问，用户可以随时说"没什么"跳过：

1. "标准做法是什么？"（或"核心 API / 步骤是什么？"）
2. "为什么这样做有效？有什么原理吗？"
...
| Phase 4 归档后查重叠 | `python tools/search-yaml.py --dir easysdd/tricks --query "{关键词}" --json` |
...
2. **status=superseded 不等于删除**。被更好方案取代的技巧保留原文，加 `superseded-by` 字段，正文顶部加一行"**[已取代]** 见 {新文档 slug}"
...
6. **可发现性是交付的一部分**。写完但没人能搜到等于没写；Phase 5 的关联推荐检查不可跳过
