...
4. **交付物复核**：检查 design 第 4 节要求归并的 architecture、requirement、roadmap 回写是否真实落盘；不要只在报告里写"已建议"
5. **diff 清洁度复核**：检查本次新增 debug 输出、临时 TODO/FIXME、注释掉代码、无用 import、方案外文件；发现就修或记录为用户确认的遗留
6. **覆盖率诚实标记**：在报告末尾写 `re-verified` 和 `trust-prior-verify` 数量。trust-prior 比例高于 30% 时，明确提醒用户哪些 UI / 手工项需要终审肉眼确认

final audit 发现任何缺口：

- 先把对应 checklist check 改回 `failed`
- 定位属于代码、方案、架构、req、roadmap 哪一类
- 修复后重新跑对应验收节和 final audit
...
- 交付物复核：architecture / requirement / roadmap {通过 / 缺口}
...
- [ ] 最终审计已完成：聚合命令重跑、交付物落盘复核、diff 清洁度复核、re-verified / trust-prior-verify 已记录；无未处理缺口
- [ ] 用户终审确认
