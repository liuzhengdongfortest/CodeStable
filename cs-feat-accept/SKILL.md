...
回看本次 feature 的实现过程，盘点是否有"每个 feature 都会撞一次"的环境 / 工具 / 工作流类信息值得沉淀到项目根的 `AGENTS.md` 或 `CLAUDE.md`。典型候选：

- 编译 / 构建命令（用什么工具、特殊参数）
- 代理 / 网络配置（npm / pip / 内网镜像 / VPN 要求）
- 本地起服务的特殊步骤（依赖某个后台进程、端口占用）
- 反复踩到的环境坑（路径编码、shell 差异、权限问题）
- 仓库内非显然的工作流约定（哪些目录由 AI 生成、提交格式）

判断标准：**下一个 feature 的 AI 还会再踩一次的事**才记。一次性踩坑、和具体业务耦合的细节不放这里（那些归 learning / decide）。

- [ ] 本次没有此类候选：写一句"本 feature 未暴露需要补入 AGENTS.md / CLAUDE.md 的内容"
- [ ] 有候选：列出来，**不擅自写入**——本节只是登记，落不落由用户在"退出后"环节定
  - 候选 1：{一句话描述 + 建议放 AGENTS.md 还是 CLAUDE.md}
  - 候选 2：...
...
7. 第 8 节（AGENTS.md / CLAUDE.md 候选盘点）——只登记不落档，等"退出后"环节问用户
8. 第 9 节（遗留）——把实现阶段攒下的"顺手发现"和已知限制都记进来
...
5. 第 8 节登记了 AGENTS.md / CLAUDE.md 候选 → 逐条问用户："候选 X：{内容}，加到 {AGENTS.md / CLAUDE.md} 吗？" 用户明确同意才写入对应文件，未同意 / 跳过的不动；全部处理完汇报"已写入 N 条 / 跳过 M 条"。**不要擅自合并多条一起加**——一条一问，用户可能只认其中一两条
6. 最后问一次是否需要代为提交本次代码和文档（scoped-commit）。用户同意时，按收尾提交规则执行到 commit 完成。
