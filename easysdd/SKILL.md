...
**子技能层**（`easysdd-{skill}/reference.md`）：各子技能专属的模板、长示例、写法说明，随技能目录一起存放。子技能引用自己的模板时写 `easysdd-{skill}/reference.md`，引用共享规则时写 `easysdd/reference/shared-conventions.md`，不再写"见根技能第五节约束 X"这类脆弱表述。
