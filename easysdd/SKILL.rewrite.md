...
- `easysdd-compound` — 回顾"做 X 时踩了 Y 这个坑"
...
compound / tricks / decisions / explore 都是存档，区别在记录内容的性质：

- 回顾某次做 X 时发现了 Y —— `easysdd-compound`
- 以后做 X 就这样做的处方 —— `easysdd-tricks`
- 全项目今后都得遵守的规定 —— `easysdd-decisions`
- 调查了一个问题，留份证据 —— `easysdd-explore`
...
目录结构（features/、issues/、learnings/、decisions/、tricks/、explores/、architecture/、tools/、reference/）的权威定义在 `shared-conventions.md`。要改目录先改那里。
