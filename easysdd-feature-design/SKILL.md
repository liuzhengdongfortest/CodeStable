...
2. **看有没有前置输入文件**——先 Glob `easysdd/features/{feature}/{slug}-intent.md` 和 `{slug}-brainstorm.md`：
   - 有 `{slug}-intent.md`：读完当作用户方案输入，后续不再重复问用户已经在 intent 里讲清楚的东西，只追问 intent 里没覆盖的角落
   - 有 `{slug}-brainstorm.md`：读完当作对话沉淀输入
   - 两者都没有：按用户当前对话里给的信息走
3. **需求输入是否清晰**——确认至少有用户目标、核心行为、成功标准、明确不做这四项（可以从 intent / brainstorm / 对话任意来源汇总）。缺了就先补；用户自己也说不清就回退到 brainstorm。
4. **对齐复杂度档位**——打开 `easysdd/reference/code-dimensions.md`，按本 feature 的场景猜一个默认组合（文档末尾有"常用默认组合"表）。然后根据已知需求和上下文，判断哪几个维度**可能偏离默认**，把偏离点和理由列成清单问用户确认。典型偏离信号：需求里提到"对外 SDK" → 可读性从 team 升到 public；需求里提到"高并发 / 低延迟" → 性能从 reasonable 升到 budgeted；纯探索脚本 → 健壮性从 L2 降到 L1。用户没明确说的维度按默认走，不要把全部 4+4 维度都摆出来让用户挨个选。确认后的结果写进第 1 节"决策与约束"的"复杂度档位"子节，**只记偏离默认的维度**。
5. **读前置资料**——动笔前必读 `AGENTS.md`、项目架构总入口、架构索引、与需求相关的现有代码和子系统架构 doc。读这一步是为了让你写的方案和现有代码能接得上，不读直接动手大概率写出脱离实际的方案。
6. **对齐 requirement**——本 feature 是在实现 / 修改哪个能力？到 `easysdd/requirements/` 下 Glob + grep 一遍，三种情况分别处理：
   - 已有对应 req：把 slug 记进方案 frontmatter 的 `requirement` 字段；动笔前读一遍 req 的"用户故事"和"边界"两节，方案不能和它们冲突。
   - 没有对应 req，但本 feature 确实新增 / 改变了一个用户可感的能力：**停下来**，提示用户先触发 `easysdd-requirements` 起草或更新 req，之后再回来继续 design。
   - 纯内部重构 / 技术债 / 工具链改造，不新增用户可感能力：不强制要 req，frontmatter `requirement` 字段留空，在第 1 节"决策与约束"里写一句"本 feature 不新增能力，无对应 requirement"。
7. **归档检索**——值不值得搜、优先搜哪些目录，规则在 `easysdd/reference/shared-conventions.md` 第 5 节。本阶段至少考虑这两个目录：
