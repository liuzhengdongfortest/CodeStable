...
```haskell
data ApiMode = Single | Batch | Incremental
data EntryStatus = Pending | Draft | Current | Outdated | Skipped Reason
data ManifestStatus = ManifestMissing | ManifestDraft | ManifestApproved | LegacyManifest
data ApiStage = InitializeManifest | GenerateSamples | GenerateEntries | UpdateEntry
data ApiOutcome = Run ApiStage | Checkpoint Reason | Complete

manifestConfirmed :: ManifestStatus -> Bool
manifestConfirmed ManifestApproved = True
manifestConfirmed LegacyManifest   = True
manifestConfirmed _                = False

approvalGate :: ApiState -> Maybe Reason
approvalGate s
  | overwritesCurrent s && not (overwriteApproved s) = Just ConfirmOverwrite
  | changesPublicContract s && not (contractWordingApproved s)
                                                    = Just ConfirmContractWording
  | otherwise                                       = Nothing

advance :: ApiMode -> ApiState -> ApiOutcome
advance _ s
  | manifestStatus s == ManifestMissing = Run InitializeManifest
  | not (manifestConfirmed (manifestStatus s)) = Checkpoint ReviewManifest
  | Just reason <- approvalGate s = Checkpoint reason

advance Single s
  | not (selectedEntryDrafted s)= Run GenerateEntries
  | not (ownerApproved s)       = Checkpoint ReviewEntry
  | otherwise                   = Complete

advance Batch s
  | not (samplesDrafted s)      = Run GenerateSamples
  | not (samplesApproved s)     = Checkpoint ReviewSamples
  | anyPending s                = Run GenerateEntries
  | not (allDraftsApproved s)   = Checkpoint ReviewAllDrafts
  | otherwise                   = Complete

advance Incremental s
  | Just entry <- nextOutdated s = Run UpdateEntry
  | not (allUpdatedEntriesApproved s) = Checkpoint ReviewAllDrafts
  | otherwise                    = Complete

mayPublish :: ApiEntry -> Bool
...
本协议核心约束：**doc-api 以源码为事实源，不靠猜，不复制上一个条目改名**。
...
3. **生成 `manifest.yaml`**——顶层 `manifest_status: draft`，所有条目初始 `status: pending`；落盘后 `validate-yaml.py --file docs/api/manifest.yaml --yaml-only` 校验
4. **`ReviewManifest`**——展示给用户 review；可标 `skipped`（内部实现）/ 调整分类 / 合并或拆分，确认后把 `manifest_status` 改为 `approved`。旧 manifest 缺该字段时按 `LegacyManifest` 兼容读取，下次写入时补齐
...
重新读源码 → 对比已有文档 → 增量更新变化部分并先写 `status: draft` → `validate-yaml.py` 校验 → `ReviewAllDrafts` → 用户确认后改 `status: current` + `last_reviewed` 当天。
...
| 来源 | 关系 |
|---|---|
| `cs-feat` acceptance 阶段 | 验收后新增/修改库公开接口 → 推送"需要更新 doc-api 吗？" |
| `cs-docs` tutorial 模式 | doc-tutorial 引用 doc-api 做详细参考；doc-api "相关条目"链回 doc-tutorial |
| `cs-feat` design 阶段 | 方案第 2 节可作 doc-api 补充信息源（**但以源码为准**） |
...
**Phase 1**：manifest.yaml 已落盘、`manifest_status: approved`（legacy manifest 可兼容读取）+ 用户已确认范围（含 skipped 理由）+ 粒度和输出路径已确认
...
**Phase 3**：outdated 条目已全部更新或确认不需更新 + 更新草稿已经用户 review + manifest 无残留 outdated（除非用户明确暂缓）
