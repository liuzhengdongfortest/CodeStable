...
```haskell
data GuideTrack = DevGuide | UserGuide
data GuideStatus = Missing | Draft | Current | Outdated
data GuideStage = CollectFacts | DraftGuide | Persist
data GuideOutcome = Run GuideTrack GuideStage | Checkpoint Reason | NeedsHuman Reason | Complete

selectTracks :: Change -> [GuideTrack]
selectTracks change = concat
  [ [DevGuide  | publicInterfaceChanged change]
  , [UserGuide | userVisibleBehaviorChanged change]
  ]

approvalGate :: GuideState -> Maybe Reason
approvalGate state
  | overwritesCurrent state && not (overwriteApproved state)
                                  = Just ConfirmOverwrite
  | changesPublicContract state && not (contractWordingApproved state)
                                  = Just ConfirmContractWording
  | otherwise                     = Nothing

advance :: GuideTrack -> GuideState -> GuideOutcome
advance track state
  | targetAmbiguous state       = NeedsHuman "which reader?"
  | Just reason <- approvalGate state
                                = Checkpoint reason
  | not (factsCollected state)  = Run track CollectFacts
  | guideStatus state `elem` [Missing, Outdated]
                               = Run track DraftGuide
  | guideStatus state == Draft && not (ownerApproved state)
                               = Checkpoint ReviewDraft
  | guideStatus state == Draft = Run track Persist
  | otherwise                  = Complete
```
...
```
python3 <cs-onboard skill 目录>/tools/search-yaml.py --dir docs/dev --filter doc_type=dev-guide --filter status=current
python3 <cs-onboard skill 目录>/tools/search-yaml.py --dir docs/user --filter doc_type=user-guide --filter component={feature-slug}
```
...
| 来源 | 关系 |
|---|---|
| `cs-feat` acceptance 阶段 | 验收后主动推：接口变更推 dev-guide，用户可见变更推 user-guide |
| `cs-feat` design 阶段 | 方案第 2 节是 dev-guide 主要信息源；第 1 节是 user-guide 主要信息源 |
| `cs-onboard` | 新仓库接入后可补全基础文档骨架 |
| `cs-keep` | dev-guide 引用的技术选型 / 用法示例若 compound 里已有沉淀，交叉引用而不重复写 |
| `cs-docs` api 模式 | guide 引用 doc-api 条目做详细参考；doc-api 是零件参考，doc-tutorial 是任务教程 |
