...
│   ├── codestable-doctor.py / codestable-workflow-next.py
│   └── codestable-spec-governance.py / codestable-dod-runner.py …
...
> `gates/`、`tools/` 与 `reference/` 由 onboard 复制，自动带上 workflow gate 配置、gate 工具与共享口径。分支与检出策略不属于 CodeStable 默认流程；需要时由宿主或独立技能决定。
...
`cs-onboard --mode refresh-runtime` 可重复执行，用来把已接入项目升级到当前技能包的 runtime。它只覆盖技能包维护的资产：`.codestable/gates/`、`.codestable/tools/`、`.codestable/reference/`、`.codestable/.gitignore` 和 `.codestable/runtime-manifest.json`；不重新审计 / 迁移文档，不移动用户文件，不改 `attention.md` 的实质内容。
...
- `.codestable/tools/`（用 `codestable-runtime-sync.py` 从当前 `cs-onboard` skill 目录复制，排除已迁出的分支 / 检出旧资产，**不要 Read 再 Write**）
...
**`.codestable/gates/`、`.codestable/tools/` 和 `.codestable/reference/` 一律用技能包新版本同步**——这些目录是技能包维护的共享资产，权威源在当前 `cs-onboard` skill 目录的 `gates/`、`tools/` 和 `references/`；项目里的只是落盘副本，且运行时目录名固定为 `.codestable/reference/`。同步会排除已迁出的分支 / 检出旧资产。技能包升级后再跑 onboard 的目的之一就是刷新副本，留旧版本会让子技能按过时口径工作。
...
- [ ] `.codestable/gates/`、`.codestable/tools/`、`.codestable/reference/` 已从技能包复制
