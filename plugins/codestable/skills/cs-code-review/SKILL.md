...
data ReviewState = ReviewState        -- 从来源 spec 产物 + git diff 恢复；只读，不落中间状态
  { source        : Maybe Source
...
data CheckpointReason
  ...
  | LaneStillPending      -- 已启动的 reviewer 未返回 / 卡住 / 权限阻塞 → status: blocked
  | SelfReviewDowngrade   -- 缺独立 Task agent 能力，需用户明确接受 self/ocr fallback 降级
```
...
selectReviewOutcome(s)
  ...
  | not s.specFinalized                                -> HumanCheckpoint SpecNotFinalized
  | not s.diffAttributed                               -> HumanCheckpoint DiffNotAttributable
  | anyStartedLanePending(s)                           -> HumanCheckpoint LaneStillPending
...
`cs-code-review` 停下的两种形态：

- `NeedsHuman`：无法开审。`.codestable/attention.md` 缺失（→ `cs-onboard`）；无可归因 diff / 定稿 spec / git range 时不做空 review，退回来源实现技能或请用户补范围。
- `HumanCheckpoint`：gate 触发人工确认，不越过继续（枚举见 `## Spec` 的 `CheckpointReason`）：来源 spec 产物未定稿（`SpecNotFinalized` → 退回来源实现技能）；diff 无法归因（`DiffNotAttributable`）；已启动的独立 reviewer 仍 pending / failed / blocked（`LaneStillPending` → 报告 `status: blocked`，不定稿 `passed`）；缺独立 Task agent 能力需用户明确接受 self/ocr 降级（`SelfReviewDowngrade`）。

两种情况都要报告：来源与 `{slug}-review.md` 路径、当前 verdict / status、阻塞或 checkpoint 原因、需要的用户决策或下一步动作（退回哪个来源实现技能 / 补什么范围 / 等哪个 reviewer）、已启动环节的状态，以及是否可安全重跑本审查。不要在环节未返回或有 blocking 时假装通过。
