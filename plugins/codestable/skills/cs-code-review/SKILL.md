...
`anyLaneFailed` 包含环节 A 的 `Blocked` / failed 和已启动环节 B 的 `OcrFailed`；失败优先于等待，必须先写 `status: blocked`，不能被 `hasBlocking` 或默认 passed 分支吞掉。`Material` / `Unknown` 永远不满足 `focusedClosureEligible`，必须重新走完整独立复审。
...
- 已有的 delivery index / validation summary / 最近实现记录（如在对话或既有产物中则按 locator 读取）；不要求为了 review 新建 implementation report
- `git status --short`
...
- goal / gate 模式下先读 evidence pack 的结论投影；只有 failed / warning / mismatch、证据不足或需要定位失败时，才下钻 gate results、DoD results 和原始证据。缺失关键证据时回 implementation gate 补证据，不现场猜测
...
2. goal / gate 模式下先读取 `{slug}-evidence-pack.md` 的状态、核心证据与 residual-risk 投影；仅在 failed / warning / mismatch、投影不足或需要定位失败时读取 `{slug}-gate-results.json`、`{slug}-dod-results.json`。关键证据缺失或 gate blocking 未解释时退回 implementation.before_review。
...
完整 frontmatter 与 verdict-sensitive 模板见同包 `references/report-template.md`（按已加载 `SKILL.md` 所在目录解析）：
- `passed` 用 `compact-passed`，保留 findings/scope/QA focus/residual risk/verdict 投影及完整 reviewer/round/lane 恢复字段。
- `changes-requested|blocked`、lane pending/failed 或待关闭 finding 用 `detailed variant`；空 finding 写 `none` 且不删章节。
focused closure 优先追加既有 detailed 报告；clean full-rereview 可改写 compact 并保留恢复锚点和 closure locator。原始 reviewer 长输出是临时传输，核验后只归并可行动 finding 与风险投影，不另存重复全文。
...
- 落盘报告前 → `references/report-template.md`（按已加载 `SKILL.md` 所在目录解析）取完整 frontmatter，并按 verdict 选择 `compact-passed` 或 `detailed variant`。
