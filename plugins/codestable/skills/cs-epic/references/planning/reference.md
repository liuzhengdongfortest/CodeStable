# Epic Planning Reference

本文件提供 epic planning 的输出格式、items schema 和 review checklist 模板。
...
> 人工 review 前必须先有 `{slug}-roadmap-review.md` 且 `status: passed`。用户修改 roadmap/items 后，实质变化要重跑 `cs-epic` review 阶段。
