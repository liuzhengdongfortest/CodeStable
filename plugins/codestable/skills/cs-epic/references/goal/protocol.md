...
每完成一个子 feature 的 design-review passed 后，立即重读 items.yaml 并继续处理下一个 planned / in-progress 子 feature；不要停下来要求用户确认该单个 design，也不要把该 design 改成 `approved`。单个 child 完成不是本阶段退出条件，不能用 final answer 把“下一步继续处理下一个 child”交还给用户；除非遇到 blocking / pending / 授权问题或用户明确要求停止，否则必须在同一轮里继续 batch loop。
