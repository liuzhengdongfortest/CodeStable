...
data NeatRequest = NeatRequest
  ...
  , decision  : Maybe NeatDecision
  }

data Phase = SizeCheck | Enumerate | ImpactMatrix | Edit | SelfCheck | Summary
                    -- Phase 0        1              2       3      4          5
data NeatDecision = ApproveLargeDocSplit | SkipLargeDocSplit | ApproveExternalMemoryEdit | SkipExternalMemoryEdit
...
runNeat req facts
  ...
  | largeDocSplitNeeded facts && decision req `notElem` [Just ApproveLargeDocSplit, Just SkipLargeDocSplit]
                                                   = HumanCheckpoint ConfirmLargeDocSplit
  | externalMemoryEditNeeded facts && decision req `notElem` [Just ApproveExternalMemoryEdit, Just SkipExternalMemoryEdit]
                                                   = HumanCheckpoint ConfirmExternalMemoryEdit
  | otherwise                                      = Completed (runPipeline [SizeCheck, Enumerate, ImpactMatrix, Edit, SelfCheck, Summary] (applyDecision req facts))
```
