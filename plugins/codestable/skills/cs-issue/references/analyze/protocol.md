...
1. **问题报告存在且已确认**——读 `{slug}-report.md`，确认 `doc_type=issue-report` 且 `status=confirmed`，5 节都有内容。不完整 / 状态不对 → 回 `cs-issue` report 阶段。`cs-issue` report 阶段 已判走标准路径就按标准路径走，不二次改判
...
告诉用户："根因分析已就绪，方案已确认。下一步阶段 3 修复验证，进入 `cs-issue` fix 阶段。"
