...
```haskell
data DesignEntry = FormalDraft | InitializeIntent | FromRoadmap
data DesignInput = Start DesignEntry | ResumeDesign DesignDecision
data DesignWait = IndependentDesignReview
data DesignStage = DraftDesign | BuildChecklist | DesignReview
data DesignOutcome
  = Run DesignStage | WriteIntentAndStop | RouteBrainstorm | ReturnToEpic
  | Awaiting DesignWait | HumanCheckpoint Reason | ApproveAndExit | Blocked Reason

advance :: DesignInput -> DesignState -> DesignOutcome
advance input state
  | input == Start InitializeIntent        = WriteIntentAndStop
  | requirementsIncomplete state           = RouteBrainstorm
  | fromRoadmap state && not (roadmapReady state)
                                             = Blocked RoadmapItemOrDependencyNotReady
  | input == ResumeDesign RequestDesignChanges = Run DraftDesign
  | not (designComplete state)              = Run DraftDesign
  ...
  | epicChildBatch state                    = ReturnToEpic
  | input == ResumeDesign ApproveDesign     = ApproveAndExit
  | otherwise                               = HumanCheckpoint ConfirmDesign
```

`designReview state` 只从 `{slug}-design-review.md` 的 `review_state/review_reason/reviewer_id` 恢复；`awaiting-reviewer` 必须带 reviewer id，旧 `blocked` 缺 `review_state` 时 fail-closed。

`epicChildBatch` 只产出 draft design/checklist/review 并回写 items.yaml；退出前必须运行
`codestable-workflow-next.py feature --epic-child-batch`，确认 `final_answer_allowed: false` 后交回 `cs-epic`。
...
**方案深度原则**：缩范围（最小闭环 / MVP）、选实现手段（fake / mock / 正则 / 占位）或定实现深度前，先做 `.codestable/reference/solution-depth-conventions.md` 的方案深度 pre-pass——按场景论证深度，不因省事默认降级也不因求稳默认镀金；采用简化 / 替身方案时，design 显式写清候选、选它的理由（不接受"更简单"作唯一理由）和转正条件。
...
2. **编排层**——主流程 / workflow / 关键编排函数 / 控制流拓扑（线性 / 分支 / 并行 DAG / 状态机）。复杂分支、并行或状态机用主流程图；简单线性流程写明顺序即可
...
2. **扫 .codestable/ 全局输入（上下文幂等：首次读、已载复用）**——Glob `.codestable/` 发现可用目录和文档类型，按类取用；**若本会话/本阶段已加载过某项全局输入，则复用已读摘要，不重复 Glob+Read**。`epic_child_batch: true` 时 CONTEXT / adrs / compound 已由 `cs-epic` 在批量开始时统一加载，子 feature 直接复用、不重扫：
...
- 复杂分支、并行或状态机在编排层开头画 mermaid 图；简单线性流程写明顺序和免图理由
...
- **spec 覆盖率**：design 每条需求 / 成功标准都能指到 checklist 里某一步；指不到就是漏步。
- **占位符扫描**：design / checklist 无 TBD、空泛"加适当错误处理""同上一步"等未落实内容。
- **术语 / 类型一致**：同一概念前后同名；接口签名、类型在各步之间一致，不前后打架。

（与 `.codestable/reference/shared-conventions.md` 第 7 节的代码结构反射检查互补：这里查 spec↔checklist 对齐，那里查落到文件时的结构信号。）
...
发一次整体 review 提示（提示词在 reference.md 第 5 节），同时附上 Top 3 风险、关键假设、自我批判结论和 `{slug}-design-review.md` 摘要。用户提意见就改；如果修改影响名词层、编排层、验收契约、steps/checks、roadmap 契约或风险策略，必须同步更新 checklist 并回到 Phase 5 重跑 design review。收到显式 `ResumeDesign ApproveDesign` 后才把 `status` 从 `draft` 改 `approved`，不能从聊天语气猜测批准。
...
落盘后 `python3 <cs-onboard skill 目录>/tools/validate-yaml.py --file {path} --yaml-only` 校验。
...
- [ ] 第 2.1 / 2.2 用"现状 → 变化"两段式；接口有示例 + 来源位置；复杂编排有主流程图，简单线性流程已写明顺序和免图理由
