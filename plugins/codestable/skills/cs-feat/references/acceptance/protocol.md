...
Acceptance 是最终仓库审计，不是实现汇报整理：复核 design/checklist、行为验证证据与交付物，盘点领域影响，受控回写 requirement/roadmap，并把可复用知识分流。它消费 code-review/QA projection，不重复逐文件 code review；final audit 仍独立承担工作区与 cleanliness 复核。实现自述只能当线索，缺口必须修复并重验。
...
data AcceptanceOutcome
  = RouteImplementation FixKind | RouteCodeReview | RunInlineVerification
...
acceptanceAuditorGate _ _ (Active ref) _ = Awaiting (AcceptanceAuditor ref)
...
goalAuthorizationMatches :: AcceptanceInput -> AcceptanceState -> Bool
goalAuthorizationMatches (ResumeGoalAcceptance ref) s = persistedGoalAuthorization s == AuthorizationApproved ref && approvalArtifactApproved s ref "goal-acceptance"; goalAuthorizationMatches _ _ = False

accept :: AcceptanceInput -> AcceptanceState -> AcceptanceOutcome
accept input s
  | not (implementationComplete s)             = RouteImplementation CompleteFeature
  | not (currentCodeReviewPassed s)             = RouteCodeReview
  | goalMode s && verificationSource s /= PassedQA
                                                = Handoff GoalRequiresQAReport
  | verificationSource s == MissingVerification = RunInlineVerification
  | coreEvidenceMissingDueToInput s             = NeedsHuman CoreEvidenceInput
  | coreEvidenceMissingDueToEnvironment s       = NeedsHuman CoreEvidenceEnvironmentUnavailable
  | not (coreEvidencePassed s)                  = RouteImplementation QAFix
  | requirementDeltaNeeded s && not (approvedDeltaExists s)
                                                = PendingHandoff "cs-req" (requirementDeltaContext s)
  | not (roadmapWritebackValid s)               = RepairGap RoadmapState
  | not (finalAuditPassed s) && sameFinalAuditGapCount s >= 3
                                                = Handoff RepeatedFinalAuditGap
  | not (allChecksPassed s)                     = RepairGap AcceptanceCheck
  | not (finalAuditPassed s)                    = RepairGap FinalAudit
  | not (acceptanceReportExists s)              = RepairGap AcceptanceReport
  | Just fix <- requestedAcceptanceFix input     = RouteImplementation fix
  | goalMode s && not (goalAuthorizationMatches input s) = Handoff GoalAcceptanceAuthorizationMissing
  | not (goalMode s) && input /= ResumeAcceptance ApproveAcceptance = HumanCheckpoint ConfirmAcceptance
  | otherwise                                   = Complete
```
...
- 第 2 节：名词与编排（2.1 名词层 / 2.2 编排层 / 2.3 挂载点 / 2.4 推进策略 / 2.5 结构健康度）
...
5. **Gate / DoD evidence**——goal / gate 模式下先读 `{slug}-evidence-pack.md` 的状态、核心证据和 residual-risk 投影；只有 failed / warning / mismatch、blocking 未解释或证据不足时才下钻 `{slug}-gate-results.json`、`{slug}-dod-results.json`。缺失关键证据或 blocking DoD 无 pass evidence 时退回 implementation gate。
6. **验证证据来源**——独立 `cs-feat` QA 阶段 不是 standalone accept 的硬前置；但验收必须有同等强度的验证证据：
   - 已有 `{slug}-qa.md`：先读取 frontmatter 和第 1/2/5/7 节投影并复核。frontmatter 必须 `doc_type=feature-qa`、`status=passed`；failed / blocked → 退回 `cs-feat` implementation 阶段 qa-fix；status 不是 passed → 不进入验收。旧 full / detailed 报告的第 3/4/6 节只在投影不足或需要复核失败上下文时下钻。
   - Standard lane 没有 `{slug}-qa.md`：不要强制切去跑 `cs-feat` QA 阶段。按需加载同 skill 的 `references/qa/behavioral-verification.md`（按已加载 `cs-feat/SKILL.md` 所在目录解析），在本次 accept 里建立 `Inline Verification Matrix`，对照 design 第 3 节、checklist behavior checks、review Test And QA Focus / residual risk 和项目运行入口，现场运行行为验证，并把证据写入 acceptance 第 3 节和第 10 节最终审计。这个模式等价于 accept-inline QA，但不额外生成 QA 报告。
   - Goal 模式例外：`cs-feat` / `cs-epic` 的 goal 协议都明确要求生成 QA 报告；goal 模式缺 `{slug}-qa.md` 时按 goal protocol 停止，不走 accept-inline。
   - Goal 授权来源：从 goal-state 读取 ref，并复核同 unit `approval-report.md` 的 `approvals.goal-acceptance: approved`；acceptance 报告里的 ref 仅是输出证据，不能反向授权自身。
...
8. **按消费者投影加载**——human design 全文 + checklist；review 先读 frontmatter 和第 4/5/6/7 节；QA 如有先读 frontmatter 和第 1/2/5/7 节；Goal evidence 先读 evidence pack projection；Standard 无 QA 使用 accept-inline matrix。只有状态异常、投影不足或需要定位缺口时才下钻 legacy/detailed report、raw DoD/gate 结果或相关实现证据。领域/交付审计再读取第 4 节候选、`requirements/CONTEXT.md`、相关 ADR 与最终工作区事实；不把所有中间 narrative 当启动必读。
...
audit_state: not-started|awaiting|needs-owner-approval|auditor-failed|completed|blocked
audit_reason: ""       # needs-owner-approval|auditor-failed|blocked 时必填
auditor_id: ""         # awaiting 时必填
acceptance_authorization_ref: "" # Goal 模式必填，须与 goal-state 一致
accepted: YYYY-MM-DD
round: 1
---
...
**review 报告重点复核**：
- [ ] `{slug}-review.md` 第 5 节 Test And QA Focus 已逐条覆盖
- [ ] `{slug}-review.md` 第 6 节 residual risk 已逐条处理 / 明确留作用户确认遗留
...
- [ ] 默认已核对 QA 第 1/2/5/7 节投影；`Command Results / Scenario Results / Diagnostic Context（如有）` 只在投影不足或异常时下钻
- [ ] failed / blocked 项为 none
- [ ] residual-risk 已逐条处理 / 明确留作用户确认遗留，且没有承载核心验收缺口
- [ ] Evidence pack projection 已复核；raw DoD/Gate 只在异常或证据不足时下钻，blocking DoD 均有 pass evidence
...
- 新增了用户可感能力，或改了边界 / 用户故事 / pitch：**必须有 owner-approved req delta** 才机械应用（`draft`→`current` 或刷新 current，保留原始愿景 + 变更日志）；没有 approved delta 就写 `approval-report.md`，以 `PendingHandoff "cs-req" context` 保留目标、feature、delta 和恢复动作，不在 accept 里直接落 current。
...
- [ ] Standard 已获用户终审确认；Goal 已消费 goal package 中机械核验通过的独立 `ApprovalRef`
