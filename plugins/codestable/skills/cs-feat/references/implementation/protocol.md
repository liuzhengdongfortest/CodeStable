...
1. 只读取并修复 QA 报告里的行为 failed / blocked items；不要把 QA finding 扩成代码风格/架构 review，也不要借机处理新需求、非阻塞建议或 review 已接受的 residual risk。
...
所有步骤完成后，按 `support/reference.md` 的"实现完成汇报"模板在当前 run 输出。Standard lane 不为后续 review/QA 单独落盘 implementation report，并直接进入 `cs-code-review`，不额外停一个“是否开始 review” checkpoint；汇报仍须列出真实 `git status`、按步骤归类的改动、step 证据、清洁度、交付物和验收场景自检。

Goal 模式例外：把 gate/resume 必需信息归并进既有 goal state / evidence projection，不额外生成独立 implementation report；随后直接按 goal 协议进入 before_review gates 和 `cs-code-review`，只有命中 goal handoff 条件才停下交还用户。
