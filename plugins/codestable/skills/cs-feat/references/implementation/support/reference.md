...
### TDD 证据
{每个行为代码 step 的 RED / GREEN / VERIFY / REFACTOR；不适用时列 `TDD exception` + 替代证据}
...
coding feature / coding step 改变代码行为且可自动化观察时，默认按同目录 `tdd.md` 的 step 内 TDD 策略执行。不能 TDD 时在 evidence 写 `TDD exception` 和替代证据。它只约束当前 step 里的测试-实现微循环，不改变 `cs-feat` implementation 阶段的 gate、checklist、证据、review / QA 流程。
...
- [ ] 完成汇报已输出；Standard / review-fix 进入 `cs-code-review`，qa-fix 进入 review 后再 QA；Goal 模式已写入报告 / 状态 / 证据记录并按 goal 协议继续
- [ ] 没有未处理的"需要叫停"信号
- [ ] 每个 step 都有退出信号证据；失败按诊断 / 窄范围修复说明 / 交还机制处理过
- [ ] 行为代码 step 有 RED / GREEN / VERIFY evidence；不适用时有 `TDD exception` 和替代证据
...
Standard 告诉用户："所有步骤完成，方案 doc 已同步。现在进入 cs-code-review。" 并在当前 run 继续，不停成新的用户 checkpoint。
...
- 把实现汇报误当成人工确认门，在 Standard 进入代码审查前多停一次
