...
description: Issue report 兼容入口。旧调用保留；执行时转入 cs-issue 主流程的 report 阶段。
---
...
入口意图：`requested_stage: report`

## 执行规则

1. 按已安装 skill 名称加载 `cs-issue` 作为权威主协议，不把兄弟目录相对路径当作主路径。
2. 在当前 run 按 `cs-issue` 主协议继续执行，并带上入口意图 `requested_stage: report`。
