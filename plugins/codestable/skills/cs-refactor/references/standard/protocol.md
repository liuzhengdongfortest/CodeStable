...
```haskell
data RefactorStage = Scan | Design | Apply | CodeReview
data RefactorOutcome
  = Run RefactorStage | Checkpoint CheckpointReason | Route RefactorRoute
  | OfferNoChange | OfferFastForward Int | Complete
data RefactorRoute
  = FeatureOrIssue Intent | RefusalRoute RefusalDestination | StandardFromFastforward
data StepPhase = StepApplied | VerificationPassed | VerificationFailed
data StepTransition = Stay RefactorStage | Continue | PersistAndCheckpoint CheckpointReason

advance :: StandardState -> RefactorOutcome
advance s
  | changesObservableBehavior s = Route (FeatureOrIssue (intent s))
  | Just refusal <- firstBlockingRefusal s = Route (RefusalRoute (refusalDestination refusal))
  | not (scanExists s)          = Run Scan
  | candidateCount s == 0       = OfferNoChange
  | candidateCount s < 3 && fastforwardEligible s
                                  = OfferFastForward (candidateCount s)
  | not (selectionConfirmed s)  = Checkpoint ScanSelection
  | not (designApproved s)      = Run Design
  | Just item <- appliedAwaitingHuman s = Checkpoint (HumanValidation item)
  | not (allApplyStepsPassed s) = Run Apply
  | not (fullVerificationPassed s) = Run Apply
  | not (codeReviewPassed s)    = Run CodeReview
  | finalValidationRequired s && not (finalValidationConfirmed s)
                                  = Checkpoint FinalValidation
  | otherwise                   = Complete

afterStep :: StepId -> VerificationOwner -> StepPhase -> StepTransition
afterStep _ _ VerificationFailed                  = Stay Apply
afterStep _ AIVerification StepApplied             = Stay Apply
afterStep _ AIVerification VerificationPassed     = Continue
afterStep item HumanVerification StepApplied       = PersistAndCheckpoint (HumanValidation item)
afterStep _ HumanVerification VerificationPassed  = Continue
```
...
apply 完成、收尾提交前必须进入 `cs-code-review` 做独立 diff 评审；blocking 未清零、important 未处理或未被 owner 明确接受时不算完成。需要 commit 时按仓库既有提交规范或 owner 指示执行。

---

## Fastforward 模式（小重构）

只有 `fastforwardEligible` 成立的 1-2 个候选才建议 fastforward；候选虽少但需要目视、改公开接口、无 safety net 或跨模块时，仍允许继续 standard，不得卡在“不要做或 fastforward”二选一。

适用话术："小重构"、"快速重构"、"简单优化下 XX 函数"、"直接改"、"别那么多步骤"。

fastforward 的唯一 eligibility 定义在 `../fastforward/protocol.md`；本协议不复制阈值。拒绝 ff 后继续本 standard 流程。ff 开干后发现变复杂，也按该协议持久化 handoff 后从 scan 恢复。
...
阶段产物依次是 scan.md、refactor-design.md + checklist.yaml、代码 + apply-notes.md。
checkpoint 由 `advance` / `afterStep` 决定：scan 要用户勾选，design 要整体放行，apply 仅 HUMAN 验证项停等。
...
动笔扫之前先跑一遍。命中任何一条 → **中止 scan，给路由建议**，不要硬凑。7 条检查和输出格式见 `../library/refusal-routing.md`。
...
- **Architecture deepening**：shallow module、pass-through wrapper、seam 泄漏、假 adapter、测试越过 interface、locality 缺失。候选项分类写"架构"，字段按 `../library/scan-checklist-format.md` 的架构扩展。

完整方法库在 `../library/methods.md`、`../library/methods-l4.md` 和 `../library/methods-architecture.md`，扫描时全量加载作匹配表。
...
2. **清单条目**（一条一块）：字段顺序和硬约束见 `../library/scan-checklist-format.md`
...
- 完成后先进入 `cs-code-review` 做独立 diff 评审；blocking 未清零、important 未处理或未被明确接受时不进 commit，scoped-commit 发起权归 `cs-code-review`
- 跑全量测试 + 类型检查 + lint
...
- `../fastforward/protocol.md` — 小重构超轻量通道
- `../library/scan-checklist-format.md` — scan 清单条目字段 / 顺序 / 硬约束
- `../library/refusal-routing.md` — scan 前置检查 7 条 + 路由表
- `../library/methods.md` — 方法库（L1-L3）
- `../library/methods-l4.md` — 方法库（L4 性能与异步）
- `../library/methods-architecture.md` — 方法库（architecture deepening）
