...
L4 只在**外部行为契约保持不变**时属于 refactor。进入方法前先写清输出、错误、顺序、允许的
时延、loading、数据新鲜度、可访问性和打印/搜索约束；任一约束会变化就停止并路由 `cs-feat`
（新行为）或 `cs-issue`（修错），不能用“性能更好”覆盖可观察差异。
...
- **不适用**：单次操作本身就够便宜；或会改变延迟、顺序、原子性、失败隔离
- **步骤**：
  1. 在操作和执行之间加一层缓冲
  2. 按时间窗口 / 容量触发批执行
...
- **不适用**：对所有用户都立即需要的核心内容；或需要新增/改变用户可见 loading、fallback、错误状态
- **步骤**：
...
- **不适用**：写密集场景加索引反降性能；没有既定 freshness/失效契约的缓存会改变读语义
- **步骤**：
...
- **不适用**：任务极短且不可取消；或异步化会改变调用顺序、错误传播、完成时机
- **步骤**：
...
- **不适用**：列表短 / 行高不稳定；或无法保持 Ctrl+F、可访问性、打印和滚动恢复契约
- **步骤**：
...
### M-L4-08 Loop Fusion 循环融合

- **适用**：同一纯数据集合被连续 filter / map / reduce 多次遍历，且可在一次遍历中保持顺序与输出
- **不适用**：各阶段有副作用、依赖中间数组身份、短路时机不同，或融合后可读性明显下降
- **步骤**：列出每次遍历的不变量 → 写等价输入输出测试 → 合并为单次 reduce/循环 → 对比结果与 benchmark
- **风险点**：过滤和转换顺序变化；稀疏数组、异常和短路语义漂移
- **验证**：边界输入输出逐项等价；benchmark 证明遍历/分配下降
- **前后端**：通用
- **配哪种 scan 项**：同一大集合的多次连续纯遍历
