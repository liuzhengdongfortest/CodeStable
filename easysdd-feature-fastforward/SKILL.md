...
> 共享路径与命名约定看根技能 `easysdd-core` 第二节。本节只补充 fastforward 特有信息。
...
**确认后立即从 design.md 抽行动清单，落到同目录 `checklist.yaml`**。清单格式和生命周期看 `easysdd-core/reference/shared-conventions.md`，本阶段只负责初始生成。提取规则：
...
文件路径：方案 doc 和 checklist.yaml 都在同一 feature 目录下；feature 目录不存在就在这一步建。位置看根技能 `easysdd-core` 第二节"目录安排"。
