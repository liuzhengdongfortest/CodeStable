"""M4 自指 + 自治：cs-skill-lab 评自己（无递归）+ 实验队列 run-next 幂等。"""
...
def test_selfref_experiment_runs(tmp_path):
    exp = ROOT / "experiments/cs-skill-lab-001"
    out = tmp_path / "r.json"
    rc = runner_mod.main(["--experiment", str(exp), "--harness", "mock", "--k", "1", "--out", str(out)])
    assert rc == 0
    data = json.loads(out.read_text(encoding="utf-8"))
    assert data["skill_under_test"] == "cs-skill-lab"          # 指向自己
...
def test_selfref_does_not_invoke_cs_skill_tools(tmp_path):
    # 自指靠「把 cs-skill-lab/SKILL.md 当被测文本注入」，不调用其工具 → 不会递归起 runner。
    # 机械证据：被测文本来自快照读取，prompt 里含 SKILL.md 标记而非工具执行结果。
    from config import ExperimentConfig, resolve_variant_text
    cfg = ExperimentConfig(name="cs-skill-lab-001", skill_under_test="cs-skill-lab")
    text = resolve_variant_text(cfg, "baseline", ROOT)
    assert "cs-skill-lab" in text and "runner.py" in text  # 是 SKILL.md 文本，不是执行产物
