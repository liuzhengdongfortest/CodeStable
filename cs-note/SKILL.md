...
按项目实际有的来，**不主动新建、不替用户拍偏好**：

- 项目根只有一个（`AGENTS.md` 或 `CLAUDE.md`）→ 写已有的那个，不问
- 两个都有 → **问用户** "本次写到 `AGENTS.md` 还是 `CLAUDE.md`？" 等用户回复再动
- 两个都没 → **问用户** "项目还没有常驻上下文文件，要建哪个？`AGENTS.md` / `CLAUDE.md` / 其他文件名（如 `.cursorrules`）？" 用户答复后再建

不主动定默认——AI 工具自己注入的提示词会告诉模型该读哪个，工具偏好该由用户和工具决定，cs-note 不替用户预设。
