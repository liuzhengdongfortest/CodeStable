...
description: Feature 工作流阶段二：按方案 doc 分步实现代码，完成后输出汇报。触发场景："方案确认了开始实现"、"按方案写代码"、"开工"，且方案 doc（含第 7 节不变量）已存在。前置依赖 easysdd-feature-design。
---
...
### 推进顺序退出信号核对
{对照方案第 6 节每个步骤的退出信号逐条勾选}

### 不变量自检
{方案第 7 节每条不变量——当前实现是否满足?靠什么保证(类型系统 / 测试 / 运行时 assert)?}
```
